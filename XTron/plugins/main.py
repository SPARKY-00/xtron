...
from XTron import (OWNER_ID, StartTime, rizoelversion, ALIVE_PIC, ALIVE_MSG, PING_MSG, HEROKU_API_KEY, HEROKU_APP_NAME, BOT_TOKEN, Tron)
...
pongg = PING_MSG if PING_MSG else "✘ XTron ✘"
RIZ_PIC = ALIVE_PIC if ALIVE_PIC else "https://te.legra.ph/file/cf5b95cafcd712ae25067.jpg"
Alivemsg = ALIVE_MSG if ALIVE_MSG else "XTron Bot is Alive..!💞"
...
async def ping(e):
    if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        if e.reply_to_msg_id:
            fuk = await e.respond("Pong!!.....", reply_to=e.reply_to_msg_id)
        else:
...
async def alive(event):
   if event.sender_id in SUDO_USERS or event.sender_id in DEV:
       ...
       rizoel += f"✘ **XTron Version**  : `{rizoelversion}`\n"  
       if BOT_TOKEN:
           Findme = await Tron.get_me()
           Name = Findme.first_name
           username = Findme.username
           rizoel += f"✘ **Assistant:** __On__ \n"
           rizoel += f"    x Name: {Name} \n    x UserName: @{username}\n"
       else:
           rizoel += "✘ **Assistant:** __Off__\n"
       rizoel += f"✘ **Owner** : [SPARKY](https://t.me/SPA4KY)\n"
...
ii) {hl}bigspam <count> <reply any message>
...
**pornspam**: Spams Porn For a given Counter. 
**COMMANDS:**
i) {hl}pornspam <counter>

**packspam**: Spams all stickers from sticker pack.
**COMMANDS:** 
i) {hl}packspam (reply to any sticker)
...
**COMMANDS:**
•) {hl}tspam <time in seconds> <count> <Message or reply to Message>
"""
...
Example: group Link : https://t.me/+Abcde
cmd : {hl}pjoin Abcde

__Only Owner And Full Sudo Users can Use!!__
"""
...
**Example**: {hl}help <category> **to get all commands in that category and its usage**
"""
...
async def stats(event):
   ...
   if event.sender_id == OWNER_ID or e.sender_id in DEV:
       ...
       # await event.edit("`Processing..`")
        ...
        rizoel += f"🤖**HERE IS YOUR XTron STATS** 🤖\n\n"
        rizoel += f"`Users:`\t**{u}**\n"
        rizoel += f"`Groups:`\t**{g}**\n"
    ...
    if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        ...
        if len(Rizoel) == 2:
            ...
            if re.search(TheRiZoeL.lower(), message.lower()):
                return await e.reply("**Sorry !!** I can't Spam On @SPA4KY , Because he is my owner.")
...
async def hang(e):
    usage = f"**Module Name:** \n\n Cmd : `{hl}hang <count>`"
    if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        Tron = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        if len(Tron) == 1:
            counter = int(Tron[0])
            if int(e.chat_id) in GROUP:
                text = f"Sorry !! I can't spam here"
...
async def tspam(e):
   ...
   if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        ...
        if len(RizoelOk) == 2:
            ...
            if re.search(TheRiZoeL.lower(), message.lower()):
                return await e.reply("**Sorry !!** I can't Spam On @SPA4KY")
            if int(e.chat_id) in GROUP:
                return await e.reply("Sorry !! I can't spam here")
...
async def delayraid(event):
   ...
   if event.sender_id in SUDO_USERS or event.sender_id in DEV:
         if event.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
         Tron = "".join(event.text.split(maxsplit=1)[1:]).split(" ", 2)
         if len(Tron) == 3:
             user = str(Tron[2])
             if int(event.chat_id) in GROUP:
             ...
             if int(e) in RiZoeLX:
                 ...
                 counter = int(Tron[1])
                 sleeptimet = sleeptimem = float(Tron[0])
                 for _ in range(counter):
                      reply = random.choice(RAID)
                      caption = f"{username} {reply}"
